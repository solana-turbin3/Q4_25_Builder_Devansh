...
import { AnchorAmmQ4 } from "../target/types/anchor_amm_q4";
...
describe("anchor-amm-q4-25", () => {
  ...
  const program = anchor.workspace.anchorAmmQ425 as Program<AnchorAmmQ4>;
  ...
  it("Withdraws liquidity from the pool", async () => {
    ...
    assert.ok(userLpAfter.amount < userLpBefore.amount, "LP balance should decrease");
    assert.ok(userXAfter.amount > userXBefore.amount, "User X balance should increase");
    assert.ok(userYAfter.amount > userYBefore.amount, "User Y balance should increase");
    
    console.log(`LP tokens burned: ${BigInt((userLpBefore.amount - userLpAfter.amount).toString())/SIX_DECIMAL}`);
    console.log(`X received: ${BigInt((userXAfter.amount - userXBefore.amount).toString())/SIX_DECIMAL}`);
    console.log(`Y received:", ${BigInt((userYAfter.amount - userYBefore.amount).toString())/SIX_DECIMAL}`);
  });
});